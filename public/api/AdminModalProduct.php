...
if ($method === 'POST') {
    ...
    if (isset($data['name'], $data['price'], $data['brand'], $data['product_class_id'], $data['details'], $data['stock'])) {
        // 新增商品
        $stmt = $conn->prepare('INSERT INTO product (name, price, brand, product_class_id, details, stock) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute([$data['name'], $data['price'], $data['brand'], $data['product_class_id'], $data['details'], $data['stock']]);
