...
export default {
  ...
  data() {
    return {
      currentCardBackground: Math.floor(Math.random() * 25 + 1), // 只是為了好玩 :D
      cardName: "", // 卡片持有人姓名
      cardNumber: "", // 卡號
      cardMonth: "", // 月份
      cardYear: "", // 年份
      cardCvv: "", // 安全碼
      minCardYear: new Date().getFullYear(), // 最小年份為當前年份
      isCardFlipped: false, // 卡片是否翻面
      focusElementStyle: null, // 聚焦元素的樣式
      isInputFocused: false // 是否聚焦輸入
    };
  },
  mounted() {
    // 組件掛載後自動聚焦卡號輸入框
  ...
  computed: {
    ...
    minCardMonth() {
      // 獲取最小月份，如果選擇的是當前年份，則不能選擇比當前月份小的月份
...
.card-input {
  ...
  &__input {
    ...
    &.-select {
      ...
      -webkit-appearance: none; /* 針對 Safari 和 舊版 Chrome */
      -moz-appearance: none; /* 針對舊版 Firefox */
