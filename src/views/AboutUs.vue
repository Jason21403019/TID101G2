<template>
  <main class="wrapper">
    <!-- logo -->
    <section :id="about_logo" class="aboutBackground_1" ref="section1">
      <img class="logo-w" :src="logoSrc" alt="" />
    </section>

    <!-- 緣由 -->
    <section class="aboutBackground_2" ref="section2">
      <span>
        <p>{{ englishText }}</p>
        <h1>{{ title1 }}</h1>
    ...
    <!-- 跳轉心理測驗 -->
    <section class="aboutBackground_3" ref="section3">
      <span>
        <p>{{ englishText }}</p>
        <h2>{{ title2 }}</h2>
...
export default {
  ...
  const: (app = Vue.createApp({
    ...
    mounted() {
      ...
      //         scrollTrigger: this.$refs.section1,
      //         duration: 2,
      // x
      //       });
